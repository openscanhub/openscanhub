...
class TestTaskInfoCommand(unittest.TestCase):
    def setUp(self):
        parser = optparse.OptionParser()
        parser.add_option = MagicMock()

        self.command = Task_Info(parser=parser)
        self.command.hub = MagicMock()
        self.command.container = MagicMock()
        self.command.normalized_name = "task-info"
    ...
    def test_runs_successfully_with_valid_task_id_and_no_json_flag(self):
        ...
        with patch('sys.stdout', new=StringIO()) as fake_out:
            ...
            self.assertIn("id = 123456", output)
            self.assertIn("owner = admin", output)
            self.assertIn("state = 1", output)
            self.assertIn("'nvr': 'foo-1.0.0-1.el8', 'koji_profile': 'brew'", output)
