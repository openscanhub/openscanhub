    ...
    def construct_cmd(self, analyzers, profile, profile_url, output_path):
        if profile == "cspodman":
            ...
            if profile_url:
                ...
                cmd += ' --scrub-on-exit'
            if profile:
                sanitized_profile = shlex.quote(profile)
                cmd += f" -r {sanitized_profile}"
                # autogenerated mock configs end with .cfg, and already have unique roots
                if not profile.endswith(".cfg"):
                    # so that scans have unique root directories, even if sharing a mock config
                    cmd += f" --root-override={sanitized_profile}-{uuid.uuid4()}"
